...
async def get_current_user_optional(request: Request):
    """Return current user or None without raising authentication errors.

    This helper reads the Authorization header directly and returns the User
    if a valid token is provided. If no header or token is invalid/expired,
    it returns None instead of raising an exception. This is useful for
    endpoints that should work for both authenticated and unauthenticated
    requests (like resume analysis in a quick-fix flow).
    """
    ...
    try:
        ...
        # Do not raise; return None to allow unauthenticated access
        return None
...
# Affinda Resume Analysis
class AffindaResumeResponse(BaseModel):
    success: bool
...
@app.post("/api/analyze-resume-affinda", response_model=AffindaResumeResponse)
async def analyze_resume_affinda(
    ...
    """
    Analyze resume using Affinda API
    """
    print(f"Affinda API Key: {'Set' if AFFINDA_API_KEY else 'Not Set'}")
    print(f"Affinda Workspace ID: {'Set' if AFFINDA_WORKSPACE_ID else 'Not Set'}")
    print(f"Target Role: {target_role}")
    print(f"Job Description provided: {'Yes' if job_description else 'No'}")
    
    if not AFFINDA_API_KEY or not AFFINDA_WORKSPACE_ID:
        error_msg = "Affinda API not properly configured. Please check your environment variables."
        print(error_msg)
        return JSONResponse(
            status_code=500,
    ...
    try:
        ...
        print(f"Sending request to Affinda API with file: {file.filename} ({len(file_content)} bytes)")
        
        # Updated Affinda API endpoint
        ...
        print(f"Affinda API response status: {response.status_code}")
        ...
        ats_score = 75  # Base ATS score, will be adjusted based on completeness
        skills = []
        experience = []
        education = []
        ...
        # Extract skills
        if data.get("skills"):
            skills = [{"name": skill.get("name"), "experience": skill.get("monthsExperience")} 
                     for skill in data.get("skills", [])]
            ats_score += 5  # Boost score for having skills
        ...
        if data.get("workExperience"):
            ...
            ats_score += 10  # Boost score for having work experience
        
        # Extract education
        if data.get("education"):
            education = [
                {
                    "degree": edu.get("accreditation"),
                    "institution": edu.get("organization"),
                    "dates": f"{edu.get('startDate', 'N/A')} - {edu.get('endDate', 'N/A')}",
                    "location": edu.get("location")
                }
                for edu in data.get("education", [])
            ]
            ats_score += 10  # Boost score for having education
        ...
        if job_description and skills:
            ...
            resume_skills = set(skill["name"].lower() for skill in skills)
            
            # Extract keywords from job description
            job_keywords = set(re.findall(r'\b\w+\b', job_description.lower()))
            job_keywords = {word for word in job_keywords if len(word) > 2}  # Filter out small words
            
            # Calculate match score
            matching_skills = resume_skills.intersection(job_keywords)
            job_match_score = len(matching_skills) / len(job_keywords) if job_keywords else 0
            
            # Identify missing keywords
            important_keywords = {word for word in job_keywords 
                               if word not in ['and', 'the', 'for', 'with', 'job']}
            missing_keywords = list(important_keywords - resume_skills)
        ...
        return {
            ...
            "education": education,
            "job_match_score": job_match_score if job_match_score is not None else None,
        ...
        job_skills = set()
        common_skills = [
            'python', 'java', 'javascript', 'react', 'node', 'mongodb', 'sql', 'docker', 'aws',
            'typescript', 'angular', 'vue', 'express', 'django', 'flask', 'kubernetes',
            'ci/cd', 'git', 'agile', 'scrum', 'rest', 'graphql', 'machine learning',
        ...
        if job_skills:
            ...
            job_match_score = len(matching_skills) / len(job_skills)
            missing_keywords = list(job_skills - resume_skills)
        
        # Extract relevant information
        analysis = {
            "success": True,
            "score": data.get("meta", {}).get("probability"),
            "job_match_score": job_match_score,
            "missing_keywords": missing_keywords,
            "skills": [{"name": skill["name"], "experience": skill.get("monthsOfExperience")} 
                      for skill in data.get("skills", [])],
            "experience": [{
                "title": exp.get("jobTitle", ""),
                "company": exp.get("organization", ""),
                "dates": f"{exp.get('startDate', '')} - {exp.get('endDate', 'Present')}",
                "location": exp.get("location", {}).get("formatted", ""),
                "description": exp.get("jobDescription", "")
            } for exp in data.get("workExperience", [])],
            "education": [{
                "degree": edu.get("accreditation", {}).get("education", ""),
                "institution": edu.get("organization", ""),
                "dates": f"{edu.get('startDate', '')} - {edu.get('endDate', 'Present')}",
                "location": edu.get("location", {}).get("formatted", "")
            } for edu in data.get("education", [])],
